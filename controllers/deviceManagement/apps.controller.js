...
export const getUserDevices = async (req, res) => {
    try {
        ...
        const armoirs = await Armoire.find({user: userId}) 
        const iotDevices = await IoTDevice.find({user: userId})
        const transformators = await Transformateur.find({user: userId})
